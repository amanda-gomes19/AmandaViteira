...
//a conexão e os atributosBd serão utilizados nas transações envolvento mais de um objeto.
    ...
    function __construct() {
        ...
        try {
            ...
            die("N&atildeo foi poss&iacute;vel conectar ao SGBD. Contate o analista respons&aacute;vel pela FSW.");
        }
    }
    ...
    /*
     * Método para retornar um array contendo os dados que estão no banco
     */

    function executaBusca($query) {
...
//Preparação
        ...
        try {
            ...
            $executou = $this->pdoStatment->execute();
            if ($executou) {
                $this->geraLogDeExecucao($query, 'executaPs');
                $dados = mysqli_fetch_all($executou);
                return $dados;
            } else {
                $this->geraLogDeErro($query, $this->getBdError());
                return null;
